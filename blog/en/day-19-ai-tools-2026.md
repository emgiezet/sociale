---
day: 19
title: "The AI Development Stack We Actually Run in Production"
pillar: Educator
language: en
...
Every few weeks, a new AI tool gets announced with marketing copy claiming it's "the last tool you'll need for AI development." Every few months, the tool that was going to replace everything quietly becomes one of six things in a complex stack.
...
**Evaluate based on measured outcomes, not demos.** Every tool in our stack survived an evaluation against our actual use case, not a synthetic benchmark.

## The Stack

### 1. AWS Bedrock — Foundation Model API and Managed RAG
...
### 5. pgvector on PostgreSQL — Vector Search for Simpler Use Cases
...
### 6. Bielik — Polish-Language LLM

Bielik is an open-source Polish LLM trained on Polish-language data. We use it primarily for embedding Polish insurance documents, where it outperforms multilingual models on terminology precision — specifically, 12 percentage points better retrieval precision on Polish insurance-specific queries.
...
### 7. Claude Code / Cursor — AI-Assisted Development
...
I wrote about vibecoding earlier in this series. Short version: use them, but understand what they're not doing for you.

## What Didn't Make the Cut

**Pinecone**: Excellent managed vector database with strong performance. But the cost at our scale, combined with the additional vendor relationship and compliance review, didn't justify the performance advantage over pgvector for our current query volume.

**Weaviate**: Strong open-source option with excellent multi-tenancy support. But the operational complexity of self-hosting exceeded the value for our use case when Bedrock Knowledge Bases covered most of our needs.

**LangChain as primary orchestration**: We use LangChain's components (document loaders, text splitters, some chains), but we've removed it as the primary orchestration layer in most of our pipelines. The abstraction was obscuring what was happening in ways that made debugging harder.
